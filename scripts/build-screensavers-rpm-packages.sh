#!/bin/bash
# Builds RPM packages for all 10 screensavers, cloning each
...
build_single_rpm() {
    ...
    echo "=========================================="
    echo "Building RPM Package via cargo-generate-rpm: $saver"
    echo "=========================================="
    if [ ! -d "$dir/.git" ]; then
        rm -rf "$dir"
        git clone "https://github.com/local76/screensavers-$saver.git" "$dir"
    fi
    (cd "$dir" && cargo generate-rpm -o "$OUTPUT_DIR")
}
